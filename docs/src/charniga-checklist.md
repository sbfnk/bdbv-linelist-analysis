...
| # | Charniga 2024 item | Status | Where in this repo | Note |
|---|---|---|---|---|
| 6 | Report measures of central tendency and variability (mean **and** SD/variance/dispersion) | 🟡 | [Analysis walkthrough — Headline estimates](analysis.md#Headline-Gamma-estimates) and [— Derived (convolved) marginals](analysis.md#Derived-(convolved)-marginals) | Median and mean reported for every atomic and convolved delay. Variability is conveyed through 95% credible intervals on those statistics and through the posterior-predictive plots, but a standalone SD / dispersion column is not currently included in the headline table. |
...
| 10 | Report study-sample characteristics (sample size, age, sex, location, route of exposure, vaccination status if any) | 🟡 | [README — Data provenance](https://github.com/epiforecasts/bdbv-linelist-analysis#data-provenance) and [Limitations — Data](limitations.md#Data) | n = 52 (BDBV subset of Rosello 2015), outbreak location (Isiro, Haut-Uélé, DRC, 2012), case-definition mix, HCW status, age (3 of 52 missing → imputed at sample mean), and sex (40 F / 12 M) are recorded in the line list and used in the model. Route of exposure is not recorded in the deposit; no vaccine was available in 2012 (vaccination status is not applicable). |
